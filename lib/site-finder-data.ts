...
const propertySelect = "*,property_scores(*),property_utility(*),property_environmental(*),property_regulatory(*),property_market(*)";

export async function getProperties(options?: { search?: string; stage?: string; county?: string; page?: number; pageSize?: number }) {
  ...
  let query = supabase.from("properties").select(propertySelect, { count: "exact" }).is("archived_at", null).order("updated_at", { ascending: false }).range((page - 1) * pageSize, page * pageSize - 1);
...
export async function getProperty(id: string) { const supabase = await createClient(); if (!supabase) return null; const { data } = await supabase.from("properties").select(`${propertySelect},property_notes(*),property_contacts(*,contacts(*)),documents(*),tasks(*),property_offtaker_matches(*,offtakers(*))`).eq("id", id).single(); return data as unknown as (PropertyRecord & Record<string, unknown>) | null; }
