...
test("regional map explains the buyer opportunity and qualification model", () => {
  ...
  for (const detail of ["County cluster", "Evaluation focus", "No project announced", "Oklahoma, Canadian, Logan", "McCurtain"]) assert.match(section, new RegExp(detail, "i"));
...
test("Why NSoul section explains benefits for businesses of different sizes", () => {
  ...
  assert.match(styles, /opacity: \.5/);
  assert.match(styles, /opacity: \.68/);
});
