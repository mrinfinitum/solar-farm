...
export function AssessmentEditor({ propertyId }: { propertyId: string }) {
  const initial = PRELIMINARY_CATEGORIES.map(([category, label]) => ({ category, rawScore: null, sourceQuality: "unknown" as const, critical: ["grid_and_interconnection", "usable_land_and_geometry", "environmental_constraints"].includes(category), explanation: `${label} has not been assessed.` }));
  ...
  return <section className="finder-card assessment-editor">
    ...
    <div className="assessment-grid">
      {PRELIMINARY_CATEGORIES.map(([category, label, weight], index) => <div className="assessment-row" key={category}>
        <div><strong>{label}</strong><span>Maximum {weight} points</span></div>
        <label><span>Raw score</span><input className="finder-field" type="number" min="0" max="100" value={components[index].rawScore ?? ""} onChange={(event) => update(index, { rawScore: event.target.value === "" ? null : Number(event.target.value) })} /></label>
        <label><span>Quality</span><select className="finder-field" value={components[index].sourceQuality} onChange={(event) => update(index, { sourceQuality: event.target.value as PreliminaryComponentInput["sourceQuality"] })}><option value="unknown">Unknown</option><option value="estimated">Estimated</option><option value="verified">Verified</option></select></label>
