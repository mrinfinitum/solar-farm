...
type Run = { id: string; property_id: string; status: string; summary?: string | null; started_at: string; successful_provider_count: number; warning_count: number; failed_provider_count: number; proposed_change_count: number; property_enrichment_steps?: Step[]; property_field_proposals?: Proposal[] };
...
export function ScreeningWorkspace({ propertyId, initialRun, canDecide }: { propertyId: string; initialRun: Run; canDecide: boolean }) {
  ...
  const process = useCallback(async () => {
    setProcessing(true); setError(""); let complete = false;
    while (!complete) {
      ...
      if (!response.ok) { setError(body.error || "A screening step failed."); break; }
  ...
  return <div className="screening-workspace">
    ...
    <section className="finder-card"><div className="finder-card-head"><div><p className="finder-eyebrow">Provider progress</p><h2>{processing ? "Screening in progress…" : "Screening sources"}</h2></div><span>{steps.filter(step => ["complete","warning","unavailable","failed","skipped"].includes(step.status)).length}/{steps.length}</span></div><div className="provider-step-grid">{steps.map(step => <article key={step.id} className={`provider-step provider-step--${step.status}`}><span>{step.status}</span><h3>{step.capability.replaceAll("_", " ")}</h3><p>{step.warning || step.error_message || (step.reused_cached_result ? "Current cached result reused." : step.status === "complete" ? "Source returned a preliminary result." : "Waiting to run.")}</p></article>)}</div></section>
