...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [data, assignees] = await Promise.all([getProperties({ search: params.q, status: params.status, county: params.county, grade: params.grade, risk: params.risk, assignedTo: params.assigned, verified: params.verified, sort: params.sort, page }), getAssignableProfiles()]);
  ...
  return <>
    ...
    <div className="finder-toolbar property-filter-toolbar"><form>
      ...
      <select className="finder-select" name="verified" defaultValue={params.verified || ""}><option value="">Any verification</option><option value="verified">Verified</option><option value="unverified">Unverified</option></select>
      <select className="finder-select" name="sort" defaultValue={params.sort || "updated_at:desc"}><option value="updated_at:desc">Recently updated</option><option value="total_acres:desc">Largest acreage</option><option value="asking_price:asc">Lowest price</option><option value="price_per_acre:asc">Lowest $/acre</option></select>
      <button className="finder-button">Apply</button>
    </form></div>
    {data.properties.length ? <><PropertyList properties={data.properties} canEdit={canEdit} /><div className="finder-pagination"><span>Showing {(page - 1) * 25 + 1}–{Math.min(page * 25, data.count)} of {data.count}</span><div>{page > 1 && <Link className="finder-button" href={`?page=${page - 1}`}>Previous</Link>} {page * 25 < data.count && <Link className="finder-button" href={`?page=${page + 1}`}>Next</Link>}</div></div></> : <div className="finder-empty"><strong>No matching properties</strong><p>Adjust the filters or create a manual property record. No listing or parcel data is inferred automatically.</p></div>}
  </>;
}
